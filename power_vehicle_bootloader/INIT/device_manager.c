...
static void SetIntervalTimerCall(void)
{
	for (UINT8 i = 0; i < SET_INTERVAL_BUFF_SIZE; i++) {
		...
		if (g_devSetInterval[i].timeCnt >= g_devSetInterval[i].DevSetInterval) {
			...
			g_devSetInterval[i].DeviceSetIntervalCall();
		}
	}
}

static INT32 CreateSetIntervalTimer(void)
{
...
{
	for (UINT8 i = 0; i < (TAG_DEVICE_END - TAG_DEVICE_START); i++) {
		g_deviceInfo[i]->fops->init(g_deviceInfo[i]);
	}
	CreateSetIntervalTimer();
}
